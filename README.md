...
`NotificationListenerService` 监听目标时钟应用通知，在通知内容命中用户配置的关键词、且“今天”属于调休日历中的休息日时，尝试触发通知里已有的跳过或关闭操作。
...
5. 根据需要点击主页面的“关键词”“跳过按钮文本”或“调休日历”进行配置。
...
- `HolidayRepository`：提供今天是否为休息日的判断，并暴露当前调休日历数据。
-
...
- 调休日历只保存休息日集合，不保存工作日、调休补班日或默认周末规则。
