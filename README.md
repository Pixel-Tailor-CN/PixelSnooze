...
- 支持临时休息模式；开启期间只把今天按休息日处理，关闭或到期后继续使用原有休息日规则。
- 支持通过首页状态卡、快捷设置磁贴和应用图标长按快捷方式开关临时休息模式。
- 临时休息模式开启时可显示静默状态通知，并提供关闭和修改时长入口。
- 应用内置一份 `assets/holiday.json`，首次运行或本地无缓存时使用内置数据。
- 用户保存自定义数据地址或手动更新调休日历后，应用会优先读取本地缓存数据。
- 启动时不会在后台自动拉取云端调休日历。
...
5. 根据需要点击主页面的“临时休息模式”“关键词”“跳过按钮文本”“休息日规则”或“调休日历”进行配置。
6. 如需快速开关临时休息，可在首页添加“临时休息”快捷设置磁贴，或长按应用图标使用快捷方式。
...
4. 命中关键词后，先检查临时休息模式，再根据当前休息日规则判断今天是否休息。
...
1. 本地缓存：用户保存自定义数据地址或手动更新后保存到应用私有目录的 `holiday.json`。
...
用户也可以在设置页配置自己的云端节假日数据地址，用于港澳地区、美国或其他地区的节假日数据。自建数据格式和维护建议见
[自建节假日数据指南](docs/custom-holiday-data.md)。
...
│   └── RestScheduleRule.kt
├── temporaryrest
│   ├── TemporaryRestManager.kt
│   ├── TemporaryRestPreferencesRepository.kt
│   ├── TemporaryRestState.kt
│   ├── TemporaryRestStatusNotification.kt
│   └── TemporaryRestTileService.kt
└── ui
    ├── home
    │   ├── CustomScheduleCalendar.kt
    │   ├── HomeScreen.kt
    │   ├── RestScheduleDialog.kt
    │   └── TemporaryRestCard.kt
    ├── settings
    │   └── SettingsScreen.kt
    └── theme
        └── Theme.kt
